        ...
        if stu_info['status'] == 'success':
            ...
            def auto_study(header, courseId):
                get_class_info = '{"operationName":"SimpleResourceTree","variables":{"courseId":' + str(
                    courseId) + '},"query":"query SimpleResourceTree($courseId: BigInt!) {\\n  tree(input: {courseId: $courseId}) {\\n    treeList {\\n      id\\n      name\\n      isGroup\\n      progress\\n      resourceType\\n      level\\n      resourceStudyStatus\\n    }\\n    __typename\\n  }\\n}"}'
                get_class_info_url = 'https://wsp.njude.com.cn/teachingApi/graphql/SimpleResourceTree'
                study_log_url = 'https://wsp.njude.com.cn/teachingApi/graphql/CreateStudyLog'
                study_progress_url = 'https://wsp.njude.com.cn/teachingApi/graphql/LearnResource'
                ...
                for k in class_info['data']['tree']['treeList']:
                    ...
                    if k['resourceType'] == 'VIDEO' and (not status):
                        ...
                        while not status:
                            post_data = (
                                    '''{"operationName":"CreateStudyLog","variables":{"input":{"app":"学习平台-WEB","courseId":''' + str(
                                courseId) + ''',"action":"PLAY","resourceId":''' + str(k[
                                                                                           'id']) + ''',"value":1,"currentTime":59.976358}},"query":"mutation CreateStudyLog($input: StudyActionLogInputObject) {\\n  createStudyLog(input: $input) {\\n    studyLog {\\n      id\\n      __typename\\n    }\\n    __typename\\n  }\\n}"}''').encode(
                                'UTF-8')
                            ...
                            post_data = '''{ "operationName": "LearnResource", "variables": { "input": { "courseId":''' + str(
                                courseId) + ''', "resourceId": ''' + str(k[
                                                                             'id']) + ''' } }, "query": "query LearnResource($input: LearnResourceInput!) {\\n  learnResource(input: $input) {\\n    id\\n    name\\n    resourceType\\n    content\\n    progress\\n    resourceStudyStatus\\n    preResource {\\n      id\\n      name\\n      __typename\\n    }\\n    nextResource {\\n      id\\n      name\\n      __typename\\n    }\\n    ...LiveResource\\n    ...FileResource\\n    ...MediaResource\\n    __typename\\n  }\\n}\\n\\nfragment LiveResource on Resource {\\n  id\\n  startTime\\n  endTime\\n  relatedSectionId\\n  playbackVideoUrl\\n  canPlayback\\n  picture\\n  url\\n  studyStatus\\n  __typename\\n}\\n\\nfragment FileResource on Resource {\\n  id\\n  name\\n  format\\n  url\\n  size\\n  downloadQuantity\\n  resourceType\\n  exerciseGroupId\\n  __typename\\n}\\n\\nfragment MediaResource on Resource {\\n  id\\n  url\\n  currentTime\\n  __typename\\n}" }'''
